$.fn.paginate = function(options) {
     ...
     return this.each(function() {
          ...
          $(document.createElement("DIV")).addClass("pagination")[(page.parent.is('TABLE')) ? 'insertAfter' : 'appendTo'](this);

          page.linkContainer = (page.parent.is('TABLE')) ? page.parent.next('.pagination:first') : page.parent.find('.pagination:first');

         $(document.createElement("A")).addClass("pagination-link previous").attr('href', 'previous').attr('title', 'Previous page').attr('rel', 'nofollow').text('<').appendTo(page.linkContainer);
          ...
          for(var num=0;  num < page.total;   num++){
               ...
               $(document.createElement("A")).addClass("pagination-link numeric").attr('href', offset).attr('title', 'Page '+offset+'').attr('rel', 'nofollow').text(offset).appendTo(page.linkContainer);

               page[offset] = page.content.slice(min, [max]);
          };

          $(document.createElement("A")).addClass("pagination-link next").attr('href', 'next').attr('title', 'Next page').attr('rel', 'nofollow').text('>').appendTo(page.linkContainer);
